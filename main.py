...
MENU_ANIMATION_FALLBACK_CAPTION = "🐾 <b>PUPBOT COMMAND CENTER</b> 🐾"
...
def build_menu_animation_caption(full_menu, header=""):
    if len(full_menu) <= TELEGRAM_ANIMATION_CAPTION_LIMIT:
        return full_menu
    caption = header.strip() or MENU_ANIMATION_FALLBACK_CAPTION
    if len(caption) <= TELEGRAM_ANIMATION_CAPTION_LIMIT:
        return caption
    return MENU_ANIMATION_FALLBACK_CAPTION
...
async def lounge_host(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message and update.message.text:
        ...
        if text_lower in ["/start", "/menu", "/help"]:
            ...
            animation_caption = build_menu_animation_caption(full_menu, header)
            try:
                with open(os.path.join(os.path.dirname(__file__), "assets/entrance_animation.gif"), "rb") as gif:
                    await context.bot.send_animation(chat_id=chat_id, animation=gif, caption=animation_caption, parse_mode="HTML", reply_markup=reply_markup if animation_caption == full_menu else None)
                if animation_caption != full_menu:
    ...
    if query.data in ["cmd:dashboard", "cmd:dashmode", "cmd:pupsona", "toggle_sleep"] or (is_alpha_console and query.data in ["cmd:alchemy", "cmd:antigravity"]):
        ...
        if query.data == "cmd:dashmode":
            if chat_id in dashboard_chats:
                ...
                await query.answer("🔮 Dashboard Mode DISABLED.")
            else:
        ...
        try:
            await query.edit_message_text(text=console_text, parse_mode="HTML", reply_markup=reply_markup)
        except Exception:
            try:
